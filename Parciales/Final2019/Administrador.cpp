...
void Administrador::ListRouter(uint16_t ip, uint16_t n_r) {
    ...
    auto *router = new Router(ip, n_r, this->log);
 ...
 * que correspondan. Aqui se determina el mejor camino a tomar por los paquetes.
...
void Administrador::InputToOutput(Router *router, int turno) {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        string msj =
                ...
                to_string(packmove->getOrigen()) + " | Destino: " + to_string(packmove->getDestino()) +
                " | Buffer a R" +
                to_string(next);
